...
interface AuthContextType {
  ...
  login: (u: string, p: string, captchaId?: string, captchaAnswer?: string) => Promise<{ success: boolean; error?: string }>;
  logout: () => void;
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (username: string, password: string, captchaId?: string, captchaAnswer?: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const res: any = await api.post('/auth/login', { username, password, captchaId, captchaAnswer });
      if (!res.success) {
        return { success: false, error: res.error || res.data?.error || 'Invalid credentials' };
      }
      // Keep the access token in memory; the refresh token is already set as an
      // httpOnly cookie by the server (sent automatically on every request).

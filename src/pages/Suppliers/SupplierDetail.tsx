...
import { ArrowLeft, Edit, Trash2, Wifi, WifiOff, RefreshCw, TestTube, Phone, Globe, Server, CheckCircle, Download, Copy, ToggleLeft, ToggleRight, Send, Ban } from 'lucide-react';
import { useData } from '../../store/DataContext';
import { suppliersApi, portalApi } from '../../services/api';
import { isSmpp, connectorIsBound } from '../../utils/bindStatus';
...
export const SupplierDetail: React.FC = () => {
  ...
  const isSmppSupplier = isSmpp(supplier.connection_type);
  const supplierBound = connectorIsBound(supplier.connection_type, supplier.bind_status, supplier.status === 'active');
  ...
  const handleSetStatus = async (status: 'active' | 'inactive') => {
    if (status === 'inactive' && !window.confirm('Deactivate this supplier? It will be excluded from routing and no longer used for new SMS.')) return;
    if (status === 'active' && !window.confirm('Activate this supplier? This clears the failure count and resets the bind so the health monitor re-checks it.')) return;
    setTogglingStatus(true);
    setStatusMsg(null);
    try {
      if (status === 'active') {
        await updateSupplier(supplier.id, { status: 'active', bind_status: 'unbound', consecutive_failures: 0 });
        setStatusMsg({ type: 'success', msg: 'Supplier activated — failures cleared.' });
      } else {
        await updateSupplier(supplier.id, { status: 'inactive' });
        setStatusMsg({ type: 'success', msg: 'Supplier deactivated.' });
      }
    } catch (e: any) {
      setStatusMsg({ type: 'error', msg: e?.message || 'Status change failed' });
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {supplier.status === 'inactive' ? (
            <Button variant="success" icon={<CheckCircle size={16}/>} onClick={() => handleSetStatus('active')} loading={togglingStatus}>Activate</Button>
          ) : (
            <Button variant="warning" icon={<Ban size={16}/>} onClick={() => handleSetStatus('inactive')} loading={togglingStatus}>Deactivate</Button>
          )}
          {supplier.status === 'active' && isSmppSupplier && (supplier.bind_status==='bound' ? (
      ...
      {statusMsg && (
        <div className={`p-4 rounded-lg border text-sm ${statusMsg.type === 'success' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
          {statusMsg.type === 'success' ? '✓ ' : '✗ '}{statusMsg.msg}
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-5 text-white"><Wifi size={20} className="mb-2" /><p className="text-sm opacity-80">Bind Status</p><div className="flex items-center gap-2 mt-1">{supplierBound?<Wifi size={18}/>:<WifiOff size={18}/>}<Badge variant={supplierBound?'success':'danger'}>{supplierBound ? (isSmppSupplier ? supplier.bind_status : 'active') : 'unknown'}</Badge></div></div>
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white"><Server size={20} className="mb-2" /><p className="text-sm opacity-80">Failures</p><p className={`text-2xl font-bold mt-1 ${supplier.consecutive_failures>10?'text-red-200':supplier.consecutive_failures>0?'text-yellow-200':'text-white'}`}>{supplier.consecutive_failures}<span className="text-sm opacity-70"> / {((supplier as any).max_failures>0?(supplier as any).max_failures:20)}</span>{(supplier as any).max_failures===0&&<span className="text-sm ml-2">(disabled)</span>}{supplier.consecutive_failures>=((supplier as any).max_failures>0?(supplier as any).max_failures:20)&&<span className="text-sm ml-2">⚠ BLOCKED</span>}{supplier.status==='inactive'&&<span className="text-sm ml-2">🚫 INACTIVE</span>}</p></div>
      ...
      {activeTab==='overview'&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card title="Billing"><div className="space-y-3">
          ...
          <div className="flex justify-between p-3 bg-gray-50 rounded-lg"><span className="text-gray-600">Force DLR</span><Badge variant={(supplier as any).force_dlr ? 'warning' : 'default'}>{(supplier as any).force_dlr ? `Yes (${(supplier as any).force_dlr_timeout_mode || 'random_0_5'} / ${(supplier as any).force_dlr_timeout || 150}s)` : 'No'}</Badge></div>
        </div></Card>

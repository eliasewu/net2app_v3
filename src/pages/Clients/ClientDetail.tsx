...
import { ArrowLeft, Edit, Trash2, Send, CreditCard, BarChart3, MessageSquare, Radio, PhoneCall, ToggleLeft, ToggleRight, Wifi, WifiOff, RefreshCw, Globe, CheckCircle, Copy, Ban } from 'lucide-react';
...
import { clientIsSmpp } from '../../utils/bindStatus';
...
export const ClientDetail: React.FC = () => {
  ...
  const isSmppClient = clientIsSmpp(client.smpp_username);
  const clientBound = isSmppClient ? (client as any).bind_status === 'bound' : true;
  ...
  const handleCopyToken = () => {
    if ((client as any).api_key && navigator.clipboard) {
      navigator.clipboard.writeText((client as any).api_key);
      setTokenCopied(true);
  ...
  const handleRegenerateToken = async () => {
    ...
    try {
      const res: any = await clientsApi.regenerateToken(client.id);
      if (res.success && res.data?.data?.api_key) {
        updateClient(client.id, { api_key: res.data.data.api_key } as any);
      }
    } catch (e) { console.error('Regenerate token failed:', e); }
  ...
  const handleSetStatus = async (status: 'active' | 'inactive') => {
    if (status === 'inactive' && !window.confirm('Deactivate this client? They will no longer be able to send SMS.')) return;
    if (status === 'active' && !window.confirm('Activate this client?')) return;
    setTogglingStatus(true);
    setStatusMsg(null);
    try {
      await updateClient(client.id, { status });
      setStatusMsg({ type: 'success', msg: status === 'active' ? 'Client activated.' : 'Client deactivated.' });
    } catch (e: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {client.status === 'active' && isSmppClient && (clientBound ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-5 text-white">
          ...
          <div className="flex items-center gap-2 mt-1">
            {clientBound ? <Wifi size={18} /> : <WifiOff size={18} />}
            <Badge variant={clientBound ? 'success' : 'danger'}>{clientBound ? (isSmppClient ? (client as any).bind_status : 'active') : 'unknown'}</Badge>
          </div>
        </div>
          ...
          <Card title="SMPP/HTTP Settings">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="col-span-2">
                <p className="text-gray-500 mb-1">API Token</p>
                <div className="flex items-center gap-2">
                  <code className="font-mono text-xs bg-gray-50 border border-gray-200 rounded px-2 py-1 flex-1 truncate">{(client as any).api_key || 'Not generated — click regenerate'}</code>
                  <button onClick={handleCopyToken} disabled={!((client as any).api_key)} title="Copy token" className="p-1.5 bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-40">
                    {tokenCopied ? <CheckCircle size={14} className="text-green-600" /> : <Copy size={14} className="text-gray-600" />}
                  </button>
                  <button onClick={handleRegenerateToken} disabled={regeneratingToken} title="Regenerate token" className="p-1.5 bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-40">
                    <RefreshCw size={14} className={regeneratingToken ? 'text-blue-500 animate-spin' : 'text-gray-600'} />
                  </button>
                </div>

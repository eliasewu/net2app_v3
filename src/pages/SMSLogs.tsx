...
export const SMSLogs: React.FC = () => {
  ...
  const getRejectionReason = (log: ExtendedLog): string => {
    ...
    const map: Record<string,string> = {
      ...
      DLR_NOT_RECEIVED: 'No DLR from supplier',
  ...
  const getDeliverResult = (log: ExtendedLog) => {
    ...
    if (dlr === 'UNDELIV' || dlr === 'FAILED') {
      const reason = log.error_code === 'UNDELIV' ? 'Supplier rejected' :
                     log.error_code === 'DLR_TIMEOUT' ? 'DLR timeout' :
                     log.error_code === 'DLR_NOT_RECEIVED' ? 'No DLR from supplier' :
  ...
  const columns = [
    ...
    { key:'operator', header:'Operator', render:(log:ExtendedLog) => {
      if (!log.operator && !log.country) return <span className="text-xs text-gray-400">—</span>;
      return <div className="flex flex-col">
        <span className="text-xs font-medium text-gray-700">{log.operator || log.country || '—'}</span>
        {log.country && <span className="text-[10px] text-gray-400">{log.country}{log.mcc ? ` • ${log.mcc}${log.mnc ? '/' + log.mnc : ''}` : ''}</span>}
      </div>;
    } },
  ...
  return (<div className="space-y-6">
    ...
    <Modal isOpen={!!detailModal} onClose={()=>setDetailModal(null)} title="Log Detail" size="lg">
      {detailModal && (<div className="space-y-4">
        ...
        <div className="col-span-2"><p className="text-xs text-gray-500">Operator / Destination</p>
          <p className="text-sm text-gray-700">
            {detailModal.operator || detailModal.country || '—'}
            {detailModal.mcc ? ` (${detailModal.mcc}${detailModal.mnc ? '/' + detailModal.mnc : ''})` : ''}
            {detailModal.country ? ` • ${detailModal.country}` : ''}
          </p>
        </div>

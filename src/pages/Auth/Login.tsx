...
// Math CAPTCHA is generated + validated server-side (see GET /api/auth/captcha),
// so bots can't just read the answer out of the bundled JS.
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!captchaInput.trim()) { setCaptchaError(true); return; }
    setLoading(true);
    await new Promise(r => setTimeout(r, 400));
    const result = await login(username.trim(), password, captchaId, captchaInput.trim());
    setLoading(false);
    if (result.success) { navigate('/', { replace: true }); }
    else { setError(result.error || 'Invalid credentials'); fetchCaptcha(); }
  };

  const refreshCaptcha = () => { if (!captchaLoading) fetchCaptcha(); };
        ...
        <div className="w-full max-w-md bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/20">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-3">
              <div className="flex items-center gap-3">
                <div className="bg-blue-500/20 rounded-lg px-4 py-2 text-center min-w-[80px]">
                  <span className="text-white text-lg font-bold font-mono tracking-wider select-none" style={{ textDecoration: 'line-through', textDecorationColor: 'rgba(255,255,255,0.3)' }}>
                    {captchaLoading ? '...' : captchaQuestion.replace(' = ?', '')}
                  </span>
                </div>
                <input type="text" value={captchaInput} onChange={e => { setCaptchaInput(e.target.value); setCaptchaError(false); }}
                  ...
                  placeholder="Answer" maxLength={3} inputMode="numeric" />
                <button type="button" onClick={refreshCaptcha} disabled={captchaLoading}
                  className="p-2 rounded-lg bg-white/10 hover:bg-white/20 text-blue-300 transition-colors disabled:opacity-50" title="Refresh CAPTCHA">
              ...
              {captchaError && <p className="text-red-400 text-xs mt-2 flex items-center gap-1"><AlertCircle size={12} /> Incorrect or expired answer, please try again</p>}
            </div>

  ...
  for (const t of rules) {
    switch (t.translation_type) {
      ...
      case 'keyword_replace': {
        // Replace forbidden keywords (comma-separated in match_content)
        // with a safe replacement word (replace_content). Whole-word, case-insensitive.
        const keywords = (t.match_content || '').split(',').map(k => k.trim()).filter(Boolean);
        const replacement = t.replace_content || t.target_value || '';
        if (keywords.length && result.message && replacement) {
          for (const kw of keywords) {
            try {
              ...
              console.warn('Translation engine: invalid keyword_replace keyword "' + kw + '" —', e.message);
            }
          }
        }
        break;

...
// The access token lives in module memory (never localStorage). It is set on
// login/refresh and attached to every request as `Authorization: Bearer <token>`.
// When it expires (30 min), the client silently exchanges the httpOnly refresh
// token (sent automatically as a cookie) for a new access token via /auth/refresh.
    ...
    refreshPromise = (async () => {
      try {
        const resp = await fetch(`${API_BASE_URL}/auth/refresh`, {
          method: 'POST',
          credentials: 'include', // httpOnly refresh cookie is sent automatically
        });
...
// Safely parse a response body. If the server returns HTML (nginx 502/504 error
// page, gateway hiccup) or an empty body, JSON.parse would throw a cryptic
// "unexpected character" message. Instead, surface the HTTP status + body
// snippet so the failure is actually diagnosable.
async function safeParseJson(response: Response): Promise<any> {
  ...
  try {
    ...
    throw new Error(
      ...
      (text.length ? `: ${text.slice(0, 120).replace(/\s+/g, ' ')}` : '')
    );
  }
}
...
class ApiClient {
  ...
  private async request<T>(
    ...
    // Auth endpoints must never trigger a self-refresh (avoids loops).
    ...
    try {
      ...
      // Access token expired (30-min TTL) → silently exchange the refresh token
      // for a new access token, then retry once.
      ...
      if (!response.ok) {
        // Session unrecoverable (refresh token expired too): drop the in-memory
        // access token. AuthContext's /auth/me will report unauthenticated and
        // ProtectedRoute redirects to /login on the next navigation.
...
export const authApi = {
  getCaptcha: () => api.get<{ captchaId: string; question: string }>('/auth/captcha'),

  login: (username: string, password: string, captchaId?: string, captchaAnswer?: string) =>
    api.post<{ token: string; user: any }>('/auth/login', { username, password, captchaId, captchaAnswer }),
...
export const clientsApi = {
  ...
  regenerateToken: (id: string) => api.post<{ data: { id: string; api_key: string } }>(`/clients/${id}/regenerate-token`, {}),
};
...
export const licenseApi = {
  ...
  adjustTenantVolume: (id: string, action: 'add' | 'deduct', volume: number, password: string) => api.post<any>(`/license/tenants/${id}/volume`, { action, volume, password }),

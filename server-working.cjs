...
app.post('/api/sms-sheba/send', auth, async (req, res) => {
  try {
    ...
    const apiUrl = `https://api.smssheba.com/smsapiv3?apikey=17a0c9ff557a81eccafefb624443573c&sender=8809606776010&msisdn=${msisdn}&smstext=${encodeURIComponent(message)}`;
    
    const messageId = `SMS_SHEBA_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
    
    await pool.query(
      `INSERT INTO sms_logs (message_id, client_id, destination, message, status, submit_time)
       VALUES ($1, $2, $3, $4, 'pending', NOW())`,
      [messageId, client_id, destination, message]
    );
    ...
    if (response.data.status === 0) {
      await pool.query(
        `UPDATE sms_logs SET status = 'delivered', dlr_status = 'DELIVRD', delivery_time = NOW() WHERE message_id = $1`,
        [messageId]
      );
      res.json({ success: true, message: 'SMS delivered', message_id: messageId });
    } else {

...
app.post('/api/sms/send', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO sms_logs (message_id, client_id, sender_id, destination, message, status, submit_time, force_dlr)
       VALUES ($1, $2, $3, $4, $5, 'pending', NOW(), $6) RETURNING *`,
      [messageId, client_id, sender_id, destination, message, force_dlr]
    );
...
app.post('/api/route_maps', auth, async (req, res) => {
  try {
...
app.post('/api/route_plans', auth, async (req, res) => {
  try {
...
app.post('/api/sms-sheba/send', auth, async (req, res) => {
  try {
    ...
    if (response.data.status === 0) {
      await pool.query(
        `UPDATE sms_logs SET status = 'delivered', dlr_status = 'DELIVRD', delivery_time = NOW(), supplier_response = $1 WHERE message_id = $2`,
        [JSON.stringify(response.data), messageId]
      );
      res.json({ success: true, message: 'SMS delivered', message_id: messageId, response: response.data });
    } else {
      await pool.query(
        `UPDATE sms_logs SET status = 'failed', error_message = $1 WHERE message_id = $2`,
        [`API status: ${response.data.status}`, messageId]
      );
      res.json({ success: false, error: 'SMS failed', response: response.data });
    }
  } catch (e) {

...
public class Database {
    ...
    public static String insertSmsLog(String clientSystemId, String sourceAddr, String destAddr,
        ...
        try (Connection conn = getConnection()) {
            ...
            // Return null on failure so the caller can NACK the submit_sm.
            // Previously we returned a fabricated messageId, which made the
            // gateway ACK messages that were never stored — silent data loss.
            return null;
        }
    }
    ...
    /**
     * Fetch a single pending DLR by its message_id (used by the immediate
     * REST push path — Node calls POST /dlr/push right after inserting the
     * row, so it should exist here).
     */
    public static PendingDlr getPendingDlrByMessageId(String messageId) {
        String sql = """
            SELECT id, message_id, entity_type, COALESCE(entity_id, client_id) as entity_id,
                   client_id, client_code, destination, sender_id,
                   status, dlr_receipt, submit_time
        ...
        try (Connection conn = getConnection();
            ...
            log.error("Get pending DLR by message_id error: {}", e.getMessage());
        }
        return null;
    }
     ...
     *
     * @param stillConnected true when another live session for the same client
     *        remains (several ESME boxes share one system_id). In that case the
     *        smpp_sessions row is left 'bound' — only the audit trail is written.
     */
    public static void recordClientUnbind(int clientId, String systemId, String ipAddress, int port, boolean stillConnected) {
        // Update smpp_sessions — only flip to unbound if this was the last live session
        if (!stillConnected) {
            ...
            log.info("Client #{} unbind: still_connected=true — keeping smpp_sessions 'bound' (sibling session live)", clientId);
        }
     ...
     *
     * @param stillConnected true when another live session for the same supplier
     *        remains (several gateways share one system_id). In that case the
     *        smpp_sessions row + suppliers.bind_status are left 'bound' — only
     *        the audit trail is written.
     */
    public static void recordInboundSupplierUnbind(int supplierId, String systemId, String ipAddress, int port, boolean stillConnected) {
        // Update smpp_sessions — only flip to unbound if this was the last live session
        if (!stillConnected) {
            ...
            log.info("Inbound supplier #{} unbind: still_connected=true — keeping smpp_sessions+suppliers 'bound' (sibling session live)", supplierId);
        }

...
async function heartbeat(config) {
  // MULTI-NODE: heartbeat every paired hub node (fall back to single server)
  const nodes = (config.nodes && config.nodes.length > 0)
    ? config.nodes
    : (config.serverUrl ? [{ url: config.serverUrl, username: config.username, password: config.password }] : []);

  for (const node of nodes) {
    if (!node.url || !node.username) continue;
    const auth = btoa(`${node.username}:${node.password || ''}`);

    try {
      const response = await fetch(`${node.url}/api/gateway/heartbeat`, {
        method: 'POST',
        headers: {
        ...
        for (const msg of data.pending_mt) {
          try {
            ...
            if (window.SmsGatewayPlugin) {
              ...
              await fetch(`${node.url}/api/gateway/mt-dlr`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Basic ${auth}`,
      ...
      console.error('[BG] Heartbeat failed [' + node.url + ']:', e.message);
    }
  }
}
